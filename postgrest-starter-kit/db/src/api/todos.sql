...
create or replace view evs as
select id,streetID,crime,distance from data.ev;
alter view evs owner to api; -- it is important to set the correct owner to the RLS policy kicks in

create or replace view evTypes as
select id,EVType from data.evType;
alter view evTypes owner to api; -- it is important to set the correct owner to the RLS policy kicks in
